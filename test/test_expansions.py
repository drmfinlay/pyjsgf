...
class Compilation(unittest.TestCase):
    def test_alt_set(self):
        e1 = AlternativeSet("a")
        self.assertEqual(e1.compile(), "a")
        e1.tag = "t"
        self.assertEqual(e1.compile(), "(a) { t }")

        e2 = AlternativeSet("a b")
        self.assertEqual(e2.compile(), "a b")
        e2.tag = "t"
        self.assertEqual(e2.compile(), "(a b) { t }")

        e3 = AlternativeSet("a", "b")
        self.assertEqual(e3.compile(), "a|b")
        e3.children[0].tag = "t1"
        e3.children[1].tag = "t2"
        self.assertEqual(e3.compile(), "a { t1 }|b { t2 }")
    ...
    def test_kleene_star(self):
        ...
        self.assertEqual(e3.compile(), "(a b)*")
        e3.tag = "t"
        self.assertEqual(e3.compile(), "(a b)* { t }")
    ...
    def test_required_grouping(self):
        ...
        self.assertEqual(e1.compile(), "(a)")
        e1.tag = "blah"
        self.assertEqual(e1.compile(), "(a) { blah }")
    ...
    def test_rule_ref(self):
        r = PublicRule("test", "a")
        rule_ref = RuleRef(r)
        self.assertEqual(rule_ref.compile(), "<test>")
        rule_ref.tag = "ref"
        self.assertEqual(rule_ref.compile(), "<test> { ref }")
    ...
    def test_sequence(self):
        ...
        e4 = Sequence("a", "b", "c")
        self.assertEqual(e4.compile(), "a b c")
        e4.children[1].tag = "t"
        self.assertEqual(e4.compile(), "a b { t } c")

    def test_nil_tag(self):
        e1 = Literal("a")
        self.assertIs(e1.tag, None)
        self.assertEqual(e1.compile(), "a")
        e1.tag = ""
        self.assertEqual(e1.compile(), "a {}")
        e1.tag = None
